...
Two independent views, side by side, because they answer different questions:
...
real-time feed). Refresh floor is intentionally 10s to avoid Yahoo rate
limiting. If you need true real-time, this is the wrong data source -- you
need Interactive Brokers with an active real-time market data subscription,
run locally (not on Streamlit Cloud).

Deploy this the same way as the RSI screener: push to the rsi-screener repo
(or a new repo) and point Streamlit Community Cloud at this file.
"""
...
refresh_seconds = st.sidebar.slider(
    "Refresh interval (seconds)", min_value=10, max_value=60, value=15,
    help="Floor is 10s to avoid Yahoo Finance rate limiting. This is polling, not push."
)
...
def live_panel():
    ...
    c2.metric(f"{ticker_bull}", f"{df['bull'].iloc[-1]:.2f}",
              f"track err {track_err_bull.iloc[-1]:+.2f}%")
    c3.metric(f"{ticker_bear}", f"{df['bear'].iloc[-1]:.2f}",
              f"track err {track_err_bear.iloc[-1]:+.2f}%")
